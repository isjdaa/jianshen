                                                ...
                                                <c:when test="${not empty courses}">
                                                    <c:forEach items="${courses}" var="course">
                                                        <tr>
                                                            ...
                                                            <td>${course.coach != null ? course.coach.name : '-'}</td>
                                        ...
                                        <nav aria-label="Page navigation">
                                            <ul class="pagination">
                                                <c:if test="${coursePager.current > 1}">
                                                    <li><a href="${pageContext.request.contextPath}/appointment/admin/courses?page=${coursePager.current - 1}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
                                                </c:if>
                                                ...
                                                <c:forEach var="pageNum" begin="1" end="${coursePager.totalPages}">
                                                    <c:if test="${pageNum >= coursePager.current - 2 && pageNum <= coursePager.current + 2}">
                                                        <li class="${pageNum == coursePager.current ? 'active' : ''}">
                                                            <a href="${pageContext.request.contextPath}/appointment/admin/courses?page=${pageNum}">${pageNum}</a>
                                                        </li>
                                                    </c:if>
                                                </c:forEach>

                                                <c:if test="${coursePager.current < coursePager.totalPages}">
                                                    <li><a href="${pageContext.request.contextPath}/appointment/admin/courses?page=${coursePager.current + 1}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
                                                </c:if>
                                            </ul>
                                        </nav>
