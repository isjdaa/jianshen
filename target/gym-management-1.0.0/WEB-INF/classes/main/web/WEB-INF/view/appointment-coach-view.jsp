                            ...
                            <div class="card-body">
                                                ...
                                                <c:when test="${not empty appointments}">
                                                    <c:forEach items="${appointments}" var="appointment">
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <c:set var="st" value="${fn:trim(appointment.status)}"/>
                                                                <c:if test="${st == 'pending' || st == '待确认'}">
                                                                    <div class="btn-group">
                                                                ...
                                                                <c:if test="${st == 'completed' || st == 'cancelled' || st == '已完成' || st == '已取消'}">
                                                                    <button class="btn btn-default btn-sm" disabled>
                                                                        <i class="mdi mdi-calendar-check"></i> 已处理
                                                                    </button>
                                                                </c:if>
                                                                <%-- 兜底：如果状态值异常，至少显示一个禁用按钮，避免“操作列空白” --%>
                                                                <c:if test="${st != 'pending' && st != '待确认' && st != 'confirmed' && st != '已确认' && st != 'completed' && st != '已完成' && st != 'cancelled' && st != '已取消'}">
                                                                    <button class="btn btn-default btn-sm" disabled title="未知状态：${st}">
                                ...
                                <c:if test="${appointmentPager != null && appointmentPager.total > 0}">
                                    <div class="text-center">
                                        ...
                                        <p class="text-muted">共 ${appointmentPager.total} 条记录，第 ${appointmentPager.current}/${appointmentPager.pages} 页</p>
                                    </div>
                                </c:if>
                            </div>
                        </div>
